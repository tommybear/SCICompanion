...
- **Phase 3 Tooling**: CLI exports are palette-aware and now include opcode/plane summaries (`--pic-summary`), draw-order samples, and pixel-diff comparisons (`--compare`, `--compare-baseline`). Remaining work: curate baseline sets and add opcode-stream dumps.
