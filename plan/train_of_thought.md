...
- **Phase 3 Tooling**: CLI exports are palette-aware and now include opcode/plane summaries (`--pic-summary`), draw-order samples, and pixel-diff comparisons (`--compare`, `--compare-baseline`). Baseline PNGs live under `Companion.Maui/tests/Baselines/Pic`. Remaining work: integrate baselines into CI and add opcode-stream dumps.
