    ...
    if (Ok == bitmap->LockBits(&rect, ImageLockModeRead | ImageLockModeWrite, PixelFormat8bppIndexed, &bitmapData))
    {
        ...
        for (int i = 0; i < usedColors; i++)
        {
            if (i != cel.TransparentColor) {
                ...
                //if (newPaletteColors[i].rgbRed == palette.Colors[cel.TransparentColor].rgbRed && newPaletteColors[i].rgbGreen == palette.Colors[cel.TransparentColor].rgbGreen && newPaletteColors[i].rgbBlue == palette.Colors[cel.TransparentColor].rgbBlue)
                //argb = (rgb.rgbRed << RED_SHIFT) | (rgb.rgbGreen << GREEN_SHIFT) | (rgb.rgbBlue << BLUE_SHIFT) | (0x00 << ALPHA_SHIFT);
                pcp->Entries[i] = argb;
            }
        }
...
bool Save8BitBmpGdiP(const char* filename, const Cel& cel, const PaletteComponent& palette, bool squishPalette)
{
    ...
    std::unique_ptr<Bitmap> bitmap = CelAndPaletteToBitmap(cel, palette, false);
